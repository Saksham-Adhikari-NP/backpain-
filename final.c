...
/* function that creates a new environment that is a duplicate of the original one, including all symbols and values. */
lenv* lenv_copy(lenv* e) {
...
/*It converts the Q-expression into an S-expression (evaluated list) and then evaluates it using the lval_eval function. */
...
/* registers a built-in function in the Lisp environment (lenv). It assigns a function (func) to a symbol (name)  */
...
/* reads a sequence of Lisp values until it reaches a designated terminating character, and then it returns an expression 
...
/* Parses a single Lisp value from the input string, handling different data types (S-expressions, Q-expressions, symbols, strings) and skipping 
whitespace/comments. */
lval* lval_read(char* s, int* i) {
